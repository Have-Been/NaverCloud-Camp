...
public class _04_ReturnLambda {
	...
	public static void main(String[] args) {
		...
		// 2. 리턴값이 있는 람다식2
		// 처리부에 중괄호 블록을 사용하면 항상 리턴 구문을 사용해줘야 한다.
		// 중괄호블록에서 return 구문을 사용하지 않으면 에러가 발생한다.
		// 처리하는 내용이 한 줄이거나 여러 줄일때 모두 사용 가능
//		result = conc((s1, s2) -> {s1 + s2});
		result = conc((s1, s2) -> {return s1 + s2;});
		System.out.println(result);
		
		// 3. 처리하는 내용이 여러줄 일 때는 중괄호 블록을 생략하면 에러가 발생한다. 
//		result = conc((st1, st2) -> 
...
//	);
		...
		// 처리할 내용이 여러 줄 일때는 중괄호블록으로 묶고 리턴 구문을 사용한다.
		result = conc((st1, st2) -> {
			System.out.println(st1);
			System.out.println(st2);
			return st1 + st2;
		...
		// 5. 람다식에서는 중괄호 블록 안에서만 ;(세미콜론)을 표시한다.
		// 중괄호 블록이 아닌 곳에서 ;(세미콜론)을 사용하면 에러가 발생한다.
//		result = conc((str1, str2) -> (str1 + str2););
//		result = conc((str1, str2) -> str1 + str2;);
