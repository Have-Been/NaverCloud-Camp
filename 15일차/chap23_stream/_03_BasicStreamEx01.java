...
//		System.out.println("아반떼 모델의 총 가격은 " + priceSum);
		...
		// 항상 최종처리 단계를 추가한다.
		int priceSum = hCarList.stream().filter
				(hCar -> hCar.getModel().equals("아반떼")) // 모델명이 아반떼인 객체만 모여있는 Stream<HyundaiCar>
				// 모델명이 아반떼인 차의 가격만 모여있는 IntStream
				.mapToInt(avante -> avante.getPrice())
				// IntStream에 담겨이는 값의 총합
				.sum();
