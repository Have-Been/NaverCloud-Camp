...
// 중첩클래스를 감싸고 있는 클래스를 외부 클래스라고 부르며 중첩 클래스는 내부 클래스라고도 한다.
// 중첩 클래스의 종류는 정적 중첩 클래스, 인스턴스 중첩 클래스, 지역 중첩 클래스, 익명 중첩 클래스가 있다.

public class NestedClass {
	// 1. 멤버 변수(인스턴스 변수, 어트리뷰트, 속성, ...)
	private int num1;
	public int num2;
	...
	// 외부 클래스의 static 키워드로 생성된 변수와 메소드에만 접근할 수 있다.
	// 중첩 클래스 자체가 static으로 컴파일러가 메모리에 바로 올리가고
	// 외부클래스의 멤버변수나 인스턴스 메소드는 객체가 생성된 후에 사용할 수 있기 때문이다.  
	// 외부클래스의 객체 생성 없이도 객체를 만들 수 있다. 
	public static class Mul{
		// 5-1. 정적 중첩 클래스의 멤버 변수
		public int num4;
		public int num5;
